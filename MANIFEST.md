...
This manifest lists the seed fixtures used to review the DI schemas, the cross-stack integration envelope, the closed-loop cycle chain, the Recovery Decision Matrix, recovery decision → execution binding, execution receipts, observed state effects, evidence freshness, and use-time authority binding.

The fixtures are intentionally small. They are not a full validation suite yet. They provide concrete examples for schema validation, semantic handoff checks, cycle continuity checks, recovery provenance checks, recovery-path admissibility checks, decision-to-execution binding checks, observed-execution receipt checks, state-effect checks, freshness/generation checks, authority revocation checks, CLI checks, and CI integration.
...
| `fixtures/valid-decision-transition-cycle-chain-recovery.json` | cycle-chain + binding validators | pass | Demonstrates preserved failure followed by a recovery cycle whose Matrix action, use-time authority, declared execution mode, observed Execution Receipt, observed State Effect Receipt, generation, and freshness window all agree. |
...
| `fixtures/invalid-authority-revoked-before-execution.json` | `scripts/validate-authority-binding.py` | fail | Authority is active when checked, revoked one second later, and execution is attempted after revocation. |
| `fixtures/invalid-authority-generation-mismatch.json` | `scripts/validate-authority-binding.py` | fail | Recovery decision binds authority generation 7 but use-time execution observes generation 8. |
| `fixtures/invalid-authority-binding-window-expired.json` | `scripts/validate-authority-binding.py` | fail | Authority remains active but the use-time check is older than the allowed binding window at dispatch. |
...
CI runs all three validators on every push to `main` and on every pull request via:
...
Use-time authority is a separate gate from state freshness. Before active recovery execution:
...
use_time_authority_receipt.authority_status
== execution_receipt.authority_status_at_execution
== active
```

The receipt must bind to the exact recovery decision, cycle, and execution mode. Its use-time window is also bounded:

```text
use_time_authority_receipt.checked_at
<= execution_receipt.executed_at

execution_receipt.executed_at - use_time_authority_receipt.checked_at
<= use_time_authority_receipt.max_binding_age_seconds
```

If `revoked_at <= executed_at`, execution is invalid even when authority was active at the earlier check. A changed authority generation also invalidates the older grant.

`RECOVERY_CONFIRMED` therefore requires an observed Execution Receipt, an evidence-backed observed State Effect Receipt matching TIP's target state, accepted state generation/freshness, and valid use-time authority for the execution seam.
...
- [`docs/use-time-authority-binding.md`](docs/use-time-authority-binding.md) for revocation-safe use-time authority checks.
...
- use-time authority generation, revocation, and binding-age checks before execution,
